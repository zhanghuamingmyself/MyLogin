package com.zhanghuaming.mylogin;
...
//                WindowManager.LayoutParams.FLAG_TRANSLUCENT_NAVIGATION);// 沉浸式状态栏
        ...
        initView();
       MobSDK.init(this, "1ef93c59a6fb8", "1843c2b54764ee24bba2c67eaf6bb5ba");//mob的key
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 0X22 && resultCode == RESULT_OK && null != data) {
    ...
    Handler handlersms = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (result == SMSSDK.RESULT_COMPLETE) {
                if (event == SMSSDK.EVENT_SUBMIT_VERIFICATION_CODE) {
                    if (result == SMSSDK.RESULT_COMPLETE) {
                        Toast.makeText(getApplicationContext(), "短信验证成功",
                                Toast.LENGTH_SHORT).show();
                        mSignupBtn.setBackground(RegisterActivity.this
                                .getResources().getDrawable(
                                        R.color.sign_up_btn_press));
                    ...
                    TimeCountUtil timeCountUtil = new TimeCountUtil(
                            RegisterActivity.this, 60000, 1000, mSendmsgBtn);
                    timeCountUtil.start();
                    // smsReceiver = new SmsReceiver();
                    // registerBroadcastReceiver();
                    content = new SmsContent(RegisterActivity.this, new Handler(),
                            mCodeET);
                    // 注册短信变化监听
                ...
                Toast.makeText(getApplicationContext(), "错误" + data, Toast.LENGTH_SHORT)
                        .show();
            }
            //dialog.dismiss();
        }
    ...
    TextWatcher myWatcher = new TextWatcher() {
        ...
        public void afterTextChanged(Editable s) {
            if (mCodeET.getText().length() == 4
                    && judgephone(mUsernameET.getText().toString()) && success) {
                new Thread() {
                    public void run() {
                        try {
                            SMSSDK.submitVerificationCode("86", mUsernameET
                                    .getText().toString(), mCodeET.getText()
                                    .toString());
                        } catch (Exception e) {
                            e.printStackTrace();
                        } finally {
    ...
    TextWatcher myWatchers = new TextWatcher() {
        ...
        public void afterTextChanged(Editable s) {
            if (mUsernameET.getText().length() == 11) {
                ...
                mSignupBtn.setBackground(RegisterActivity.this.getResources()
                        .getDrawable(R.color.sign_up_btn_normal));
            }
            // //////////////////////////////////////////////////////////////////////////////////
        }
